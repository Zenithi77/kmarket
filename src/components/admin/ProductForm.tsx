...
export interface ProductFormInitialData {
  ...
  size_type?: SizeType;
  sizes?: string[];
  category_id?: string; // resolved plain id of whatever category_id currently points to
...
export default function ProductForm({ mode, productId, initialData }: ProductFormProps) {
  ...
  // Extra attribute options added inline by the admin (per filter key), merged
  // with the category's preset options so they're immediately selectable.
  const [customAttrOptions, setCustomAttrOptions] = useState<Record<string, string[]>>({});
  const [addingAttrKey, setAddingAttrKey] = useState<string | null>(null);
  const [newAttrOptionValue, setNewAttrOptionValue] = useState('');
  ...
  // Prefill the form once, as soon as both the categories list and the product's
  // initial data are available (category/subcategory resolution needs the list).
  useEffect(() => {
    if (!initialData || initializedFromDataRef.current || categories.length === 0) return;
    ...
    setFormData(prev => ({
      ...
      originalPrice: initialData.sale_price ? String(initialData.sale_price) : '',
      brand: initialData.brand || '',
      weight: initialData.weight ? String(initialData.weight) : '',
      stock: String(initialData.stock ?? ''),
      isNew: !!initialData.is_new,
      isFeatured: !!initialData.is_featured,
    }));

    setMedia((initialData.images || []).map((url) => ({
      url,
      type: isVideoUrl(url) ? 'video' : 'image',
    })));

    setSelectedColors(initialData.colors || []);
    setSizeType(initialData.size_type || 'none');
    setSelectedSizes(initialData.sizes || []);
    setAttributes(initialData.attributes || {});

    // Resolve which main category this product belongs to. If it has a
    // subcategory, the parent is whichever main category contains it.
    const subId = initialData.subcategory_id;
    let mainCat: CategoryOption | null = null;
    if (subId) {
      mainCat = categories.find(c => (c.subcategories || []).some(s => s._id === subId)) || null;
    } else if (initialData.category_id) {
      mainCat = categories.find(c => c._id === initialData.category_id) || null;
    }
  ...
  // Inline "add a new option" for a select/multi-select attribute filter — makes
  // it selectable immediately without needing to go edit the category elsewhere.
  const handleAddAttrOption = (key: string) => {
    const value = newAttrOptionValue.trim();
    if (!value) return;
    ...
    setAttributes(prev => ({ ...prev, [key]: value }));
    setNewAttrOptionValue('');
    setAddingAttrKey(null);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.category && !selectedMainCat) {
    ...
    try {
      ...
      toast.success(mode === 'edit' ? 'Бараа шинэчлэгдлээ' : 'Бараа амжилттай нэмэгдлээ');
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">
            {mode === 'edit' ? 'Бараа засах' : 'Шинэ бараа нэмэх'}
          </h1>
            ...
            return (
              <div className="bg-white rounded-xl card-shadow p-6 space-y-4">
                ...
                {catFilters.map((filter: { key: string; label: string; type: string; options: string[] }) => {
                  const allOptions = [
                    ...filter.options,
                    ...(customAttrOptions[filter.key] || []).filter(o => !filter.options.includes(o)),
                  ];
                  return (
                    <div key={filter.key}>
                      ...
                      {filter.type === 'select' || filter.type === 'multi-select' ? (
                        <div className="space-y-1.5">
                          ...
                          {addingAttrKey === filter.key ? (
                            <div className="flex gap-1.5">
                              <input
                                type="text"
                                value={newAttrOptionValue}
                                onChange={(e) => setNewAttrOptionValue(e.target.value)}
                                onKeyDown={(e) => e.key === 'Enter' && (e.preventDefault(), handleAddAttrOption(filter.key))}
                                placeholder="Шинэ сонголт бичих..."
                                className="flex-1 px-3 py-1.5 rounded-lg border border-gray-200 text-xs focus:border-primary-500 outline-none"
                                autoFocus
                              />
                              <button
                                type="button"
                                onClick={() => handleAddAttrOption(filter.key)}
                                className="px-2.5 py-1.5 bg-primary-500 text-white text-xs rounded-lg hover:bg-primary-600"
                              >
                                Нэмэх
                              </button>
                              <button
                                type="button"
                                onClick={() => { setAddingAttrKey(null); setNewAttrOptionValue(''); }}
                                className="px-2.5 py-1.5 text-gray-400 text-xs rounded-lg hover:bg-gray-100"
                              >
                                Болих
                              </button>
                            </div>
                            ...
                            <button
                              type="button"
                              onClick={() => { setAddingAttrKey(filter.key); setNewAttrOptionValue(''); }}
