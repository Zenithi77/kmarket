...
import { Eye, EyeOff, Mail, Lock, ArrowRight, ArrowLeft, ShoppingBag } from 'lucide-react';
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        toast.error(data.error || 'Нэвтрэх үед алдаа гарлаа');
        return;
      }
    ...
    <div className="min-h-screen bg-soft-bone flex items-center justify-center p-4">
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-gray-900 mt-4">Тавтай морил!</h1>
          <p className="text-gray-500 mt-1">Бүртгэлрүүгээ нэвтэрнэ үү</p>
        </div>
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-xl shadow-gray-200/50 p-8 border border-white">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full bg-primary-600 text-white py-4 rounded-xl font-bold hover:bg-primary-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 shadow-brand hover:scale-[1.02] active:scale-[0.98]"
            >
              {isLoading ? (
